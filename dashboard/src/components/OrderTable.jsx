      ...
      <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="pl-9 pr-3 py-1.5 text-sm border border-slate-200 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-orange-400"
          />
        </div>
          ...
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              {TABLE_HEADERS.map(({ label, align }) => (
                <th
                  key={label}
                  className={`px-2 py-2 font-medium text-slate-600 whitespace-nowrap ${
            ...
            {loading ? (
              <tr>
                <td colSpan={7} className="py-24 text-center">
                  <RefreshCw className="animate-spin mx-auto text-slate-300" size={32} />
                </td>
              </tr>
            ) : filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-24 text-center text-slate-400">
                  {error ? "Không thể tải dữ liệu" : "Không có đơn hàng nào"}
                </td>
              </tr>
            ) : (
              filtered.map((order) => (
                <tr key={order.order_id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-2 py-2 font-mono text-slate-700 whitespace-nowrap">#{order.order_id}</td>
                  <td className="px-2 py-2 text-slate-600 whitespace-nowrap">{order.user_id}</td>
                  <td className="px-2 py-2 text-slate-600 whitespace-nowrap">{order.product_id}</td>
                  <td className="px-2 py-2 text-right text-slate-600 whitespace-nowrap">{order.quantity}</td>
                  <td className="px-2 py-2 text-right font-medium text-slate-800 whitespace-nowrap">
      ...
      <div className="px-4 py-3 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-sm flex items-center gap-1 whitespace-nowrap disabled:opacity-40 hover:border-orange-300 transition-colors"
          >
